...
This article provides troubleshooting guides for issues with Azure Arc-enabled Kubernetes connectivity, permissions, and agents. It also provides troubleshooting guides for Azure GitOps, which you can use in either Azure Arc-enabled Kubernetes or Azure Kubernetes Service (AKS) clusters.

For help troubleshooting issues related to extensions, such as GitOps (Flux v2), Azure Monitor Container Insights, or Open Service Mesh, see [Troubleshoot extension issues for Azure Arc-enabled Kubernetes clusters](extensions-troubleshooting.md).
...
If the Helm Chart release is present with `STATUS: deployed`, check the status of the agents by using `kubectl -n azure-arc get deployments,pods`:
...
If you receive an overage claim, make sure that your service principal isn't part of more than 200 Microsoft Entra groups. If it is, create and use another service principal that isn't a member of more than 200 groups, or remove the original service principal from some of its groups and try again.

An overage claim can also occur if you configure an outbound proxy environment without allowing the endpoint `https://<region>.obo.arc.azure.com:8084/` for outbound traffic.

If neither of these conditions apply, [open a support request](/azure/azure-portal/supportability/how-to-create-azure-support-request) so Microsoft can look into the issue.

## Issues when connecting Kubernetes clusters to Azure Arc

Connecting clusters to Azure Arc requires access to an Azure subscription and `cluster-admin` access to a target cluster. If you can't reach the cluster, or if you have insufficient permissions, connecting the cluster to Azure Arc fails. Make sure you meet all of the [prerequisites to connect a cluster](quickstart-connect-cluster.md#prerequisites).
...
Outbound network connectivity problems can happen for many different reasons. First, make sure your environment meets all of the [network requirements](network-requirements.md).

If you encounter connectivity problems and your cluster is behind an outbound proxy server, make sure that you passed proxy parameters while onboarding your cluster and that the proxy is configured correctly. For more information, see [Connect using an outbound proxy server](quickstart-connect-cluster.md#connect-using-an-outbound-proxy-server).
...
Problems retrieving the MSI certificate usually happen because of network problems. Check to make sure your environment meets all of the [network requirements](network-requirements.md), and then try again.

### Insufficient cluster permissions

If the kubeconfig file you provide doesn't have enough permissions to install the Azure Arc agents, the Azure CLI command returns an error: `Error: list: failed to list: secrets is forbidden: User "myuser" cannot list resource "secrets" in API group "" at the cluster scope`

To fix this problem, make sure the user connecting the cluster to Azure Arc has the `cluster-admin` role assigned.

### Unable to connect OpenShift cluster to Azure Arc

If `az connectedk8s connect` times out and fails when connecting an OpenShift cluster to Azure Arc:

1. Ensure that the OpenShift cluster meets the version prerequisites: 4.5.41+ or 4.6.35+ or 4.7.18+.

1. Before running `az connectedk8s connnect`, run this command on the cluster:
...
Connecting a Kubernetes cluster to Azure Arc-enabled Kubernetes requires installing Azure Arc agents on the cluster. If the cluster runs over a slow internet connection, the container image pull for agents might take longer than the Azure CLI timeouts.

### Helm timeout error

You might see the error `Unable to install helm release: Error: UPGRADE Failed: time out waiting for the condition`. To resolve this issue, try the following steps:
...
1. Check if the `clusterconnect-agent` or the `config-agent` pods are showing `crashloopbackoff`, or whether any containers aren't running:
...
1. If the `azure-identity-certificate` isn't present, the system assigned managed identity isn't installed.
   ...
   ```
      ...
      To resolve this issue, try deleting the Arc deployment by running the `az connectedk8s delete` command, then reconnecting your cluster. If the issue continues, it might be an issue with your proxy settings. In that case, [try connecting your cluster to Azure Arc via a proxy](./quickstart-connect-cluster.md#connect-using-an-outbound-proxy-server) to connect your cluster to Arc via a proxy. Also verify that all of the [network prerequisites](network-requirements.md) are met.

1. If the `clusterconnect-agent` and the `config-agent` pods are running, but the `kube-aad-proxy` pod is missing, check your pod security policies. This pod uses the `azure-arc-kube-aad-proxy-sa` service account, which doesn't have admin permissions but requires the permission to mount host path.

1. If the `kube-aad-proxy` pod is stuck in `ContainerCreating` state, check whether the kube-aad-proxy certificate is downloaded onto the cluster.
...
When you try to onboard Kubernetes clusters to the Azure Arc platform, the local environment (such as your client console) might return the following error message:

```output
Cannot load native module 'Crypto.Hash._MD5'
```

Sometimes, dependent modules fail to download successfully when you add the extensions `connectedk8s` and `k8s-configuration` through Azure CLI or Azure PowerShell. To fix this problem, manually remove and then add the extensions in the local environment.
...
## Cluster connect problems

If your cluster is behind an outbound proxy or firewall, verify that websocket connections are enabled for `*.servicebus.windows.net`. This connection is required specifically for the [Cluster Connect](cluster-connect.md) feature. Additionally, make sure you're using the latest version of the `connectedk8s` Azure CLI extension if you're experiencing problems using cluster connect.

If the `clusterconnect-agent` and `kube-aad-proxy` pods are missing, the cluster connect feature is likely disabled on the cluster. If so, `az connectedk8s proxy` fails to establish a session with the cluster, and you might see an error reading `Cannot connect to the hybrid connection because no agent is connected in the target arc resource.`
...
This warning message appears when you use a service principal to sign in to Azure, and the service principal doesn't have the necessary permissions. To avoid this error, follow these steps:
