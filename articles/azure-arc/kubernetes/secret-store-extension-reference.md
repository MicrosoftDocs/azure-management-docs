...
`AKVSync` resources simplify the most common SSE use cases by consolidating configuration into a single resource. This has the combined benefits of being easier for a human to configure and is less prone to inconsistencies. When applied, `AKVSync` resources are translated to their equivalent `SecretSync` and `SecretProviderClass` resources. Do not modify the autogenerated `SecretSync` and `SecretProviderClass` resources, they will be updated as needed automatically.
...
Within .spec:
 ...
 - **objects** *(required)*: A list of items (**secretInAKV** or **kubernetesSecretName**). Items can be secrets to fetch from AKV, or more complex 'compound secrets' that may contain many items from AKV.
    ...
    - **kubernetesSecretName** *(optional)*: Define the name for a compound secret in Kubernetes, which can have any number of items. **versionHistory** cannot be used in this situation, if multiple versions are needed, they must be explicitly enumerated. **labels** and **annotations** can also be specified as above.
