...
Within .spec:
 ...
 - **objects** *(required)*: A list of items (**secretInAKV** or **kubernetesSecretName**). Items can be secrets to fetch from AKV, or more complex 'compound secrets' that may contain many items from AKV.
    ...
    - **kubernetesSecretName** *(optional)*: Define the name for a compound secret in Kubernetes, which can have any number of items. **versionHistory** cannot be used in this situation, if multiple versions are needed, they must be explicitly enumerated. **labels** and **annnotations** can also be specified as above.
