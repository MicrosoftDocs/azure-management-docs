...
You can install a namespace-scoped extension in a specific namespace by using the [`--target-namespace` property](extensions.md#optional-parameters-for-extension-creation). Because extension can be deployed at a namespace scope, multiple instances of a namespace-scoped extension and its components can run on a cluster. Each instance of the extension has permissions for the namespace where it's deployed.
