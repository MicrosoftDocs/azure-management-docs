...
This article provides troubleshooting tips for common problems related to [cluster extensions](extensions-release.md) like Argo CD, GitOps (Flux v2) in Azure, or Open Service Mesh (OSM).
...
If you experience an error during installation or if the extension shows a `Failed` state, ensure that the cluster doesn't have any policies that restrict creating the `flux-system` namespace or any of the resources in that namespace.
...
The controllers that you install in your Kubernetes cluster when you install the `microsoft.flux` extension need enough CPU and memory resources to schedule properly on a Kubernetes cluster node. Ensure that your cluster meets the minimum memory and CPU resource requirements.
...
This section provides troubleshooting guidance for common issues encountered when installing and using the Argo CD extension on Azure Arc-enabled Kubernetes clusters.

### Errors installing the microsoft.argocd extension

If you encounter an error during installation, or if the extension enters a Failed state, verify that the cluster doesn't have any policies that prevent the creation of the `argocd` namespace or any resources within that namespace.

### Errors installing the microsoft.argocd extension: context deadline exceeded

This error typically indicates that the Helm installation timed out while waiting for the pods to become ready. It's commonly observed when the extension is deployed with High Availability enabled on clusters with fewer than four nodes.

To resolve the issue, reinstall the extension with High Availability disabled, or ensure the cluster has sufficient capacity to support the deployment.
...
This section shows commands you can use to validate and troubleshoot the deployment of [Open Service Mesh (OSM)](tutorial-arc-enabled-open-service-mesh.md) extension components on your cluster.

> [!WARNING]
> Microsoft announced the retirement of the [Open Service Mesh (OSM) add-on for AKS](https://azure.microsoft.com/updates?id=open-service-mesh-add-on-for-aks-will-be-retired-on-september-30-2027) on September 30, 2027. The upstream OSM project has also been retired by the [Cloud Native Computing Foundation (CNCF)](https://docs.openservicemesh.io/).
...
To check the OSM controller service, run the following command:
...
If the OSM injector is healthy, you see output similar to the following example:
...
Ensure that the IP address listed for the `osm-injector` service is `9090`. There should be no value listed for `EXTERNAL-IP`.
...
If the **Validating** webhook is healthy, you see output similar to the following example:
...
If the **Mutating** webhook is healthy, you see output similar to the following example:
...
A well-configured **Validating** webhook has output similar to the following example:
...
A well-configured **Mutating** webhook has output similar to the following example:
...
Check whether the OSM controller gave the **Validating** (or **Mutating**) webhook a CA bundle by using the following command:
...
metadata:
  ...
  uid: a6a6a6a6-bbbb-cccc-dddd-e7e7e7e7e7e7
spec:
  certificate:
    certKeyBitSize: 2048
