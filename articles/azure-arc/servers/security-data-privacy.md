...
To collect cloud metadata, the Azure Connected Machine agent queries the instance metadata service (IMDS) endpoints for AWS, GCP, Oracle Cloud, Azure Local, and Azure. When the Hybrid Instance Metadata Service (HIMDS), the agent's local metadata handler, starts, it performs a one-time check to determine whether the server is running in a cloud environment. As part of this check, your security software may notice the agent reaching out to the endpoints described in the following table.
...
The following sections describe the local logs the Azure Connected Machine agent maintains.
