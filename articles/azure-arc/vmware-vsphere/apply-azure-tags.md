...
1. Sign in to the [Azure portal](https://portal.azure.com/), go to **Azure Arc** > **VMware vCenters**, and then select the VMware vCenter that manages the resource you plan to apply Azure tags to.
       To apply Azure tags to the VMware vCenter, go to the **Tags** blade in the VMware vCenter Azure resource. 
       To apply Azure tags to the associated Azure Arc resource bridge, go to its Azure resource from the **Overview** blade.
 
1. To apply tags to Virtual machines, a Resource Pool, a Cluster, a Host, VM templates, VM networks, or Datastores, go to the specific resource through the dedicated inventory views under the **vCenter inventory**.
...
>[!Important]
>Azure tags you apply on a VMware resource are accessible only from Azure and don't propagate to the VMware vSphere resource tags or custom attributes and vice versa.
