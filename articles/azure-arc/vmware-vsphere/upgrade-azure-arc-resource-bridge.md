...
You must manually upgrade the resource bridge associated with your VMware vSphere environment to keep it up to date. Microsoft triggers the upgrade of your resource bridge only when it's about to fall out of support. 

## Versions and upgrade availability

Periodically, Microsoft releases new versions of the Azure Arc resource bridge to include security and feature updates. Generally, Microsoft supports the latest released version and the previous three versions (up to n-3) of resource bridge. You must upgrade or redeploy a resource bridge on an unsupported version to be in a production support window and ensure the certificates in the resource bridge are still valid. Hence, it's highly recommended to maintain the resource bridge in the latest version and to perform upgrades every six months. To stay informed on releases, visit the [Azure Arc resource bridge release notes](/azure/azure-arc/resource-bridge/release-notes).

If your resource bridge is at version n-3, you might receive an email notification letting you know that your resource bridge goes out of support once the next version is released. If you receive this notification, upgrade the resource bridge as soon as possible to be in the latest version. If there are any problems with manual upgrade, submit a support ticket to receive assistance from Microsoft while you're still in a supported version.
...
You can manually upgrade the Azure Arc resource bridge. The upgrade command takes your Azure Arc resource bridge to the immediate next version, which might not be the latest available version. You might need multiple upgrades to reach a [supported version](/azure/azure-arc/resource-bridge/release-notes). To manually upgrade your resource bridge, follow these steps:

1. Ensure that you install the latest `az arcappliance` CLI extension by running the following extension upgrade command from the vCenter server:
     ```azurecli
...
Microsoft offers cloud-managed upgrades as a supplementary service, but this service doesn't replace the need for manual upgrades every six months. By using cloud-managed upgrade, Microsoft attempts to upgrade the Arc resource bridge if it will soon go out of support. Microsoft sets the upgrade trigger time. The upgrade prerequisites must be met for cloud-managed upgrade to work. Disruptions, errors, or unhealthy resource bridge state can cause cloud-managed upgrades to fail. Since Microsoft has negligible control over the resource bridge, which runs in an on-premises environment, periodically perform manual upgrades to ensure a supported Azure Arc resource bridge version.  

You handle cloud-managed upgrades through Azure. A notification is pushed to Azure to reflect the state of the resource bridge as it upgrades. As the resource bridge progresses through the upgrade, its status might switch back and forth between different upgrade steps. Upgrade is complete when the **resource bridge status** is *Running* and **provisioningState** is *Succeeded*. 
...
>[!NOTE]
>- It might not always be possible to upgrade an unsupported resource bridge to a newer version, as component services used by the Azure Arc resource bridge might no longer be compatible. In addition, the unsupported resource bridge might not be able to provide reliable monitoring and health metrics. 
