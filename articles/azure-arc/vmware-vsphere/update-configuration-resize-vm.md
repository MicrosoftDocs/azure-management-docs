...
1. Go to the **Virtual machines** inventory view under the vCenter inventory. Alternatively, you can go to the inventory view for VMs enabled for management in Azure from **Azure Arc** > **Machines** blade.
1. Select the machine for which you want to update the NICs and then select **Settings** > **Networking**:

   - **Add a network interface**: Select **Add network interface** and enter the required fields such as Name, VM network, and Adapter settings. Check the **Enable power on at boot** checkbox. Select **Add** and then **Save** to apply the changes.
...
1. Sign in to the [Azure portal](https://portal.azure.com/), go to **Azure Arc** > **VMware vCenter**, and then select the VMware vCenters that manages the VM you want to modify.
1. Go to the **Virtual machines** inventory view under the vCenter inventory. Alternatively, you can go to the inventory view for VMs enabled for management in Azure from **Azure Arc** > **Machines** blade.
1. Select the machine for which you want to add or remove a disk and then select **Settings** > **Disks**:

   - **Add a disk**: Select **Add disk** and enter the required fields such as Disk name, size, controller, and mode. Select **Add** and then **Apply** to apply the changes.
   
     :::image type="content" source="media/update-configuration-resize-vm/add-disk.png" alt-text="Screenshot showing add a disk screen." lightbox="media/update-configuration-resize-vm/add-disk.png":::
...
1. Sign in to the [Azure portal](https://portal.azure.com/), go to **Azure Arc** > **VMware vCenter** and then select the VMware vCenter, which manages the VM, which you're planning to modify from Azure.
1. Go to the **Virtual machines** inventory view under the vCenter inventory. Alternatively, you can go to the inventory view for VMs enabled for management in Azure from **Azure Arc** > **Machines** blade.
1. Select the machine that you want to resize. Select **Settings** > **Size**. You can change the number of CPU cores and memory size.  
