...
This article provides information on troubleshooting and resolving issues that can occur while attempting to deploy, use, or remove the Azure Arc resource bridge. The resource bridge is a packaged virtual machine that hosts a _management_ Kubernetes cluster. For general information, see [Azure Arc resource bridge overview](./overview.md).
