...
All workload orchestration resources are managed through Azure Resource Manager, enabling fine-grained [Role-Based Access Control (RBAC)](rbac-guide.md) and consistent governance. You can interact with workload orchestration using the [CLI](solution-without-common-configuration.md) and [Azure portal](azure-portal-monitoring.md), while non-code onsite staff benefit from a [user-friendly interface](portal-user-guide.md) for authoring, monitoring, and deploying solutions with site-specific configurations.
