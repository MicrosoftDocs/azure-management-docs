...
Some common cloud problems include misconfigured RBAC permissions or policy restrictions, such as:ç
...
1. To collect container logs or Kubernetes events from the Azure Arc-enabled Kubernetes cluster you need to enable [Azure Monitor for Kubernetes cluster](/azure/azure-monitor/containers/kubernetes-monitoring-enable?tabs=cli#arc-enabled-cluster-1).

1. Once you have enabled Azure Monitor for containers, you can use the following commands to collect logs and events using an existing Log Analytics workspace.
