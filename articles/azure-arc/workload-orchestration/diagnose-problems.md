...
description: Learn how to diagnose workload orchestration logs and errors, including enabling audit and diagnostic logs, and collecting container logs or Kubernetes events.
...
1. You can debug a workload orchestration operation by usign the built-in query **Workload orchestration target provider and solution deployment failures**. This query helps you to identify logs errors from the edge service. The query is as follows:
...
1. Configure your applications to send logs to the Edge Monitor pipeline by specifying the service IP of the pipeline as the endpoint for syslog (TCP) or Otel (gRPC) logs. Update your application's logging configuration to use the following endpoints:
...
1. Verify logs in the Log Analytics workspace:
    ...
    - For Otel logs from otel-log-app, use the following query in the Log Analytics workspace:
