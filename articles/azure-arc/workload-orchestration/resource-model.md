...
ms.topic: conceptual
ms.date: 05/10/2026
# Customer intent: As a platform engineer, I want to understand the resource model of workload orchestration, so that I can model environments, organise infrastructure, and deploy workloads effectively.
---

# Workload orchestration resource model

Workload orchestration uses a set of core resource types to model environments, define deployment topology, and enable application deployment across distributed clusters. These resources work together to represent both the organisational structure and the deployment endpoints for applications.
...
- **Hierarchy** – Represents the physical structure of your organisation
- **Target** – Represents the deployment unit or namespace within a Kubernetes cluster

## Context

A context, also referred to as an environment, is the top-level resource that groups all workload orchestration resources within an Azure tenant. It links to your organizational hierarchy and defines the set of capabilities available for deployment. Capability tags are name-description pairs that describe what a resource is capable of doing. They are used to map applications to be deployed, to the relevant targets.

Each Azure tenant supports a single context, which ensures consistency in how resources are organised and managed.

## Hierarchy

A hierarchy represents the organisational structure of your environment using level-based definitions such as *factory*, *region*, or *line*. It maps closely to the physical layout of your operations.

Hierarchies are defined as name-description pairs that correspond to levels in your organisation. For example:

- Factory → Line
- Region → City → Store

Hierarchies are implemented using **Sites**, which represent physical entities such as plants, factories, or stores. All levels of a hierarchy, except the leaf level, must be associated to a Site. Sites must be tagged with the correct hierarchy level so that they are interpreted correctly by workload orchestration.
...
- Proof-of-concept environments that do not require more than two hierarchy levels
...
Service group hierarchy is used for more complex, multi-level organisational structures. Service groups are tenant-level Azure Resource Manager (ARM) resource containers that allow you to organise related resources—such as resource groups, subscriptions, and deployment targets—under a unified logical structure, without changing the existing Azure resource layout. Relationships among service groups involved in the hierarchy are used to define parent-child relationships within your Site hierarchy. A service group hierarchy is suitable for:

- Enterprise-scale deployments requiring you to model real-world organisational hierarchies (for example, Region → City → Factory → Line)
...
- Up to four levels of hierarchy can be modelled.
...
2. The **hierarchy** maps the organisational structure and physical layout using sites.
