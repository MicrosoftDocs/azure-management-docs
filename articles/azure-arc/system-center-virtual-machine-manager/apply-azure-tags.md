...
- If you plan to apply Azure tags to a Virtual machine, VMM Cloud, VM template, or VM network, ensure the resource is [enabled for management in Azure](enable-scvmm-inventory-resources.md).
...
1. Sign in to the [Azure portal](https://portal.azure.com/), go to **Azure Arc** > **SCVMM management server** and then select the SCVMM server that manages the resource you plan to apply Azure tags.
       To apply Azure tags to the SCVMM server, go to the **Azure tags** blade in the SCVMM server Azure resource. 
       To apply Azure tags to the associated Azure Arc resource bridge, go to its Azure resource from the **Overview** blade.
 
1. To apply tags to Virtual machines, VMM Clouds, VM templates, or VM networks, go to the specific resource through the dedicated inventory views under the **SCVMM inventory**.
...
1. Select the resource to which you want to apply Azure tags and then select **Tags**.
1. Enter the name and value in the fields respectively and then select **Apply**.
...
>[!Important]
>Azure tags you apply on a SCVMM resource are accessible only from Azure and don't propagate to the SCVMM console resource tags or custom properties and vice versa.
