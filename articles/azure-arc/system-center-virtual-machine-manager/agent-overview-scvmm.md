...
When you [enable guest management](enable-guest-management-at-scale.md) on SCVMM VMs, Azure arc agent is installed on the VMs. By using the Azure Connected Machine agent, you can manage your Windows and Linux machines that you host outside of Azure on your corporate network or other cloud providers. This article provides an architectural overview of Azure connected machine agent.
...
When you install the Connected Machine agent for Windows, it makes the following system-wide configuration changes:
...
* Several log files are available for troubleshooting, described in the following table.
...
The Connected Machine agent for Linux is available in the preferred package format for your distribution (`.rpm` or `.deb`) in the Microsoft [package repository](https://packages.microsoft.com/). The shell script bundle [Install_linux_azcmagent.sh](https://aka.ms/azcmagent) installs and configures the agent.
...
The performance data was gathered in April 2023 on virtual machines running Windows Server 2022 and Ubuntu 20.04. Actual agent performance and resource consumption vary based on the hardware and software configuration of your servers.
...
> [!NOTE]
> Azure Arc-enabled servers doesn't store or process customer data outside the region where the customer deploys the service instance.
