...
AWS EC2 instances, EKS clusters, and GCP VMs that are connected to Azure Arc through the **[Arc onboarding](onboard-multicloud-vms-arc.md)** solution are represented as Arc-enabled server resources under `Microsoft.HybridCompute/machines` in the `<PublicCloud>_<AccountId>` resource group. If you previously onboarded an AWS EC2 instance, EKS Cluster, or GCP VM to Azure Arc, you won't see that resource in this resource group, because it already has a representation in Azure.
