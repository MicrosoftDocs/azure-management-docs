...
| `401 Token validation failed` | RBAC roles not assigned for the managed identity. | Assign `Reader` + `Cognitive Services OpenAI User` + `FoundryInferenceAccess` app role. See [Configure Foundry Local inference authentication](configure-foundry-inference-authentication.md). |
