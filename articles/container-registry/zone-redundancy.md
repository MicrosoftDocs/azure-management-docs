...
In addition to [geo-replication](container-registry-geo-replication.md), which replicates registry data across one or more Azure regions to provide availability and reduce latency for regional operations, Azure Container Registry supports optional *zone redundancy*. [Zone redundancy](/azure/reliability/availability-zones-overview#availability-zones) provides resiliency and high availability to a registry or replication resource (replica) in a specific region.
...
Use Azure [availability zones](/azure/reliability/availability-zones-overview) to create a resilient and high availability Azure container registry within an Azure region. For example, organizations can set up a zone-redundant Azure container registry with other [supported Azure resources](/azure/reliability/availability-zones-service-support) to meet data residency or other compliance requirements, while providing high availability within a region.
...
Run the [az acr replication create](/cli/azure/acr/replication#az-acr-replication-create) command to create a zone-redundant registry replica in a region that [supports availability zones](/azure/reliability/availability-zones-region-support) for Azure Container Registry, such as *westus2*. 
