...
By using [Azure role-based access control (RBAC)](/azure/role-based-access-control/), you can assign a built-in role to users, managed identities, or service principals to grant Microsoft Entra-based permissions defined within the role.
...
You can assign ACR roles to the following identity types to grant permissions to a registry:
...
- [Managed identity for Azure resources](container-registry-authentication-managed-identity.md)
  ...
  - [Azure Container Apps identity](/azure/container-apps/managed-identity-image-pull)
  - [Azure Container Instances identity](/azure/container-instances/using-azure-container-registry-mi)
  - [Azure Machine Learning workspace identity](/azure/machine-learning/how-to-identity-based-service-authentication)
  - [Azure Machine Learning attached Kubernetes cluster node kubelet identity](/azure/machine-learning/how-to-attach-kubernetes-to-workspace) to allow the Kubernetes cluster's nodes to pull images from ACR.
  - [Azure Machine Learning online endpoint identity](/azure/machine-learning/how-to-access-resources-from-endpoints-managed-identities)
...
- [Service principal](container-registry-authentication.md#service-principal)
  ...
  - [Azure Container Instance service principal](container-registry-auth-aci.md)
...
> [!NOTE]
> [Connected registries](intro-connected-registry.md) don't support Azure role assignments or Microsoft Entra-based permissions management.

## Perform role assignments to grant permissions

To perform role assignments, you must have either the `Owner` role or the `Role Based Access Control Administrator` role on the registry.
 You can make role assignments by using the [Azure portal](/azure/role-based-access-control/role-assignments-portal), [Azure CLI](/azure/role-based-access-control/role-assignments-cli), or [Azure PowerShell](/azure/role-based-access-control/role-assignments-powershell).

For information about how to assign a role to an identity, see [Steps to add a role assignment](/azure/role-based-access-control/role-assignments-steps).

## Scope role assignments to specific repositories

Use Microsoft Entra attribute-based access control (ABAC) to manage Microsoft Entra-based repository permissions. This feature lets you scope role assignments to specific repositories in a registry.
...
Alternatively, see the [Azure Container Registry roles directory reference](container-registry-rbac-built-in-roles-directory-reference.md) for a list of built-in roles that support Microsoft Entra ABAC conditions.
...
The applicable built-in roles and role behavior depend on the registry's **Role assignment permissions mode**:

- **RBAC Registry + ABAC Repository Permissions**: Supports standard RBAC role assignments with optional [Microsoft Entra ABAC conditions](container-registry-rbac-abac-repository-permissions.md). to scope assignments to specific repositories.
- **RBAC Registry Permissions**: Supports only standard RBAC assignments without ABAC conditions.

To view this setting in the Azure portal, go to your registry, and then select **Properties** under the **Settings** section in the service menu.

The following list describes common container registry scenarios and their recommended roles, depending on the registry's role assignment permissions mode.
...
- **Scenario: Identities that need to list all repositories in the registry**
  ...
  - ABAC support: This role **doesn't support Microsoft Entra ABAC conditions**. As such, this role assignment **grants permissions to list all repositories** in the registry.
...
- **Scenario: Developers and command line scripts that use `az` CLI to authenticate and login to the registry (via the `az acr login` command), as well as to run other `az` CLI commands on a registry.** Take note that identities that only need image pull, push, or delete permissions do not need the following role. This scenario is strictly about clarifying the role that grants permissions to enable running `az` CLI on a registry, including `az acr login` and other `az` commands on a registry. Take note that this role also grants permissions to create, update, or delete ACR registries, as well as granting control plane permissions to maange registry configuration (see below).
...
- **Scenario: Vulnerability scanners and tools that need to audit registries and registry configurations, as well as access to registry images to scan them for vulnerabilities**
  ...
  - ABAC support: ACR recommends that vulnerability scanners and monitors have full data plane access to all repositories in the registry. As such, assign these roles without Microsoft Entra ABAC conditions to grant role permissions without scoping them to specific repositories.
...
- **Scenario: Visual Studio Code Docker extension users**
  ...
  - Permissions: Grants capabilities to browse registries, pull and push images, and build images by using `az acr build`, supporting common developer workflows in Visual Studio Code.
  - ABAC support: ACR recommends that Visual Studio Code users have full data plane access to all repositories in the registry. As such, assign these roles without Microsoft Entra ABAC conditions to grant role permissions without scoping them to specific repositories.
