...
You can use Entra attribute-based access control (ABAC) for managing Entra-based repository permissions.
This feature allows you to scope role assignments to specific repositories in a registry.

For an overview of Entra ABAC repository permissions, including the ACR built-in roles that support Entra ABAC conditions, see [Entra-based repository permissions](container-registry-rbac-abac-repository-permissions.md).
Alternatively, you can consult the [Azure Container Registry roles directory reference](container-registry-rbac-built-in-roles-directory-reference.md) for a list of built-in roles that support Entra ABAC conditions.
...
- **Scenario: Identities that need to pull images and validate supply chain artifacts such as developers, pipelines, and container orchestrators (for example, Azure Kubernetes Service node kubelet identity, Azure Container Apps, Azure Container Instances, Azure Machine Learning workspaces)**
  ...
  - ABAC support: This role supports optional Entra ABAC conditions to scope role assignments to specific repositories in the registry.
...
- **Scenario: Identities that need to list all repositories in the registry**
  ...
  - ABAC support: This role **doesn't support Entra ABAC conditions**. As such, this role assignment will **grant permissions to list all repositories** in the registry, regardless of any Entra ABAC conditions specified in the role assignment.
...
- **Scenario: Pipelines, identities, and developers that need to create, update, or _delete_ ACR registries**
  ...
  - ABAC support: This role doesn't support Entra ABAC conditions as the role is scoped to the registry level, granting permissions to manage control plane settings and configurations for the entire registry.
...
- **Scenario: Pipelines, infrastructure engineers, or control plane observability/monitoring tools that need to list registries and view registry configurations, but not access to registry images**
  ...
  - ABAC support: This role doesn't support Entra ABAC conditions as the role is scoped to the registry level, granting permissions to read control plane settings and configurations for the entire registry.
...
- **Scenario: Vulnerability scanners and tools that need to audit registries and registry configurations, as well as access to registry images to scan them for vulnerabilities**
  ...
  - ABAC support: ACR recommends that vulnerability scanners and monitors have full data plane access to all repositories in the registry. As such, these roles should be assigned without Entra ABAC conditions to grant role permissions without scoping them to specific repositories.
...
- **Scenario: Pipelines and identities that orchestrate [ACR tasks](container-registry-tasks-overview.md)**
  ...
  - ABAC support: This role doesn't support Entra ABAC conditions as the role is scoped to the registry level, granting permissions to manage all ACR Tasks in the registry.
...
- **Scenario: Identities such as pipelines and developers that manage [ACR transfer pipelines](container-registry-transfer-cli.md) for transferring artifacts between registries using intermediary storage accounts across network, tenant, or air gap boundaries**
  ...
  - ABAC support: This role doesn't support Entra ABAC conditions as the role is scoped to the registry level, granting permissions to manage all ACR transfer pipelines in the registry.
...
- **Scenario: Management of [quarantined images](https://github.com/Azure/acr/blob/main/docs/preview/quarantine/readme.md)**
  ...
  - ABAC support: This role doesn't support Entra ABAC conditions as the role is scoped to the registry level, granting permissions to manage all quarantined images in the registry.
...
- **Scenario: Visual Studio Code Docker extension users**
  ...
  - ABAC support: ACR recommends that Visual Studio Code users have full data plane access to all repositories in the registry. As such, these roles should be assigned without Entra ABAC conditions to grant role permissions without scoping them to specific repositories.
